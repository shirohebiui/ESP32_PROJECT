...
// ===== WiFi 설정 =====
#define WIFI_SSID     "wifiid" // <<WiFi이름 수정필요
#define WIFI_PASSWORD "wifipw" // <<WiFi비밀번호 수정필요
...
String getLastValueFromAWS() {
  ...
  if (responseCode > 0) {
    ...
    int idx = response.lastIndexOf("value:");
    if (idx != -1) {
      String value = "";
      for (int i = idx + 6; i < response.length(); i++) {
        if (response[i] == '"' || response[i] == '}' || response[i] == ',') break;
        value += response[i];
      }
      Serial.println("[AWS] 마지막 value: " + value);
      http.end();
      return "{Ch:0,value:" + value + "}";
    }
  }
  Serial.println("[AWS] 값 가져오기 실패");
  http.end();
  return "";
}
...
void setup() {
  ...
  // ===== 마지막 값 복원 =====
  lastData = getLastValueFromAWS();
  Serial.println("[INIT] 복원된 lastData: " + lastData);
...
void loop() {
  if (doConnect) {
    doConnect = false;
    xTaskCreate(bleConnectTask, "ble_connect", 4096, NULL, 1, NULL);
  }
  ...
  // ===== 마지막 저장값 출력 =====
  Serial.println("[lastData] " + lastData);
